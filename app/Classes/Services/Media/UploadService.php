...
//        $this->awsOSS($local_file, $remote_path, $name_type);
        $this->aliyunOSS($local_file, $remote_path, $name_type);
    }

    private function aliyunOSS($local_file, $remote_path, $name_type = self::FILE_NAME_TYPE_RANDOM)
    {
        $accessKeyId = 'LTAI4FsTGTJ5ezHGeSwv68Pm';
        $accessKeySecret = 'KigNP0XB4RfvKjRlN2zODOmzNJXIif';
        ...
        $bucket = 'amodvis-in-one';
        try {
            if (self::FILE_NAME_TYPE_ORIGIN === $name_type) {
                $base_name = basename($local_file);
                $ext = $this->getFileExt($base_name);
            } else {
                $base_name = md5(microtime(true));
                $ext = $this->getFileExt(basename($local_file));
                $base_name .= '.' . $ext;
            }
            $tem_content_type = $this->getContentType($ext);
            $options = [];
            if ($tem_content_type) {
                $options['Content-Type'] = is_array($tem_content_type) ? implode(';', $tem_content_type) : $tem_content_type;
            }
            $ret = $ossClient->uploadFile($bucket, $remote_path . '/' . $base_name, $local_file, $options);
            var_dump($ret);
        } catch (OssException $e) {
            print $e->getMessage();
        }
    ...
    private function awsOSS($local_file, $remote_path, $name_type = self::FILE_NAME_TYPE_RANDOM)
    {
